...
func main() {
	...
	insecureSkipVerify := flag.Bool("insecureskipverify", false, "ignore server certificate")
	flag.Parse()
...
Options:
  ...
  -cacert     Required, the name of the CA that signed the server's certificate
	-insecureSkipVerify  Optional, ignores server certificate
 `
	...
	if *caCertFile == "" && !(*insecureSkipVerify) {
		log.Fatalf("caCert or ignore tls is required but missing:\n%s", usage)
	}
